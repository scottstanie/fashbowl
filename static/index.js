...
const template =
    Handlebars.compile(document.querySelector('#load-messages').innerHTML);
const template_title =
    Handlebars.compile(document.querySelector('#load-channel-title').innerHTML);

// Load current value of display_name
document.addEventListener('DOMContentLoaded', () => {
  // Connect to a web-socket. Used for sending and recieving messages
  // dynamically
  ...
  const channel_to_display =
      template_title({'channel_to_display': current_channel});
  var channel_element = channel_to_display, parser = new DOMParser(),
  ...
  function asynch_load_messages(request, refresh) {
    /*
    Parse JSON message data response from server. This functionality is used to
    generate the proper message view when the user refreshes the page, or
    switches channels.
    */
    ...
    document.querySelector('#comment-list').style.paddingTop =
        `${comment_stack[current_channel]}%`;
    var comment_count =
  ...
  function channel_switch() {
    /* Assign this function to button.onclick attribute. It the wrapper function
     * for switching rooms.
     */
    ...
    if (this.value != room_to_leave) {
      ...
      // Generate message view with data from server for user switching to a new
      // channel
      const request = new XMLHttpRequest();
      request.open('POST', '/');
      request.onload =
          asynch_load_messages.bind(null, request, refresh = false);
  ...
  // When connected to socket, configure send message button, and emit
  // message_data to FLASK server
  socket.on('connect', () => {
    socket.emit('join', current_channel);
    ...
    prevent_blank_text_entry('#word-input', '#submit-send-word');
    document.querySelector('#submit-send-message').onclick = () => {
      ...
      if ((user == 'superuser') && (message_content == delete_channel) &&
    ...
    document.querySelector('#submit-send-word').onclick = () => {
      ...
      var submitted_word = document.querySelector('#word-input').value
      var timestamp = new Date();
      timestamp = timestamp.toLocaleString('en-US');
      var user = username;
      var current_channel = localStorage.getItem('current_channel');
      ...
      socket.emit('send word', message_data);
      document.querySelector('#submit-send-word').disabled = true;
  ...
  socket.on('announce channel deletion', message_data => {
    ...
    const channel_to_display = template_title(
        {'channel_to_display': localStorage.getItem('current_channel')});
    var channel_element = channel_to_display, parser = new DOMParser(),
        doc = parser.parseFromString(channel_element, 'text/xml');
    document.querySelector('#message-view')
        .prepend(doc.querySelector('#channel-title'));
    ...
    document.querySelector('#comment-list').style.paddingTop =
        `${comment_stack[localStorage.getItem('current_channel')]}%`;
    var comment_count =
        document.querySelector('#comment-list').childElementCount;
    if (comment_count > 0) {
      document.querySelector('#comment-list').lastElementChild.scrollIntoView();
      ...
      request.onload =
          () => {
            ...
            if (data.success) {
              ...
              console.log(
                  'Server did not process request to add new channel, deleting channel UI');
